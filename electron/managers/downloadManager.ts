...
import { DownloadedMusicItem } from '../preload'
...
export class DownloadManager {
    ...
    // 生成标准化的文件名（不包含时间戳）
    static generateFileName(musicInfo: MusicInfo, extension: string = ''): string {
        if (!musicInfo) {
    ...
    // 安全处理字符串，防止 undefined 和 null
    private static safeString(str: any): string {
    ...
    // 下载单个文件（通用方法）
    static async downloadFile(options: DownloadOptions): Promise<boolean> {
    ...
    // 下载封面图片
    static async downloadCover(coverUrl: string, savePath: string, fileName: string = 'cover'): Promise<string | null> {
        if (!coverUrl) {
        ...
        try {
            ...
            const coverFilePath = path.join(savePath, coverFileName)
    ...
    // 获取图片文件扩展名
    static getImageExtension(imageUrl: string): string {
    ...
    // 检查音乐文件是否已存在
    static checkMusicExists(musicInfo: MusicInfo, savePath: string): boolean {
        const fileNameBase = this.generateFileName(musicInfo)
        const possibleExtensions = ['.mp3', '.flac', '.m4a', '.wav']

        for (const ext of possibleExtensions) {
            const filePath = path.join(savePath, fileNameBase + ext)
            if (this.fileExists(filePath)) {
                return true
            }
        }
    ...
    // 获取音乐文件扩展名
    static getMusicExtension(musicUrl: string): string {
        if (musicUrl.includes('.flac')) return '.flac'
        if (musicUrl.includes('.m4a')) return '.m4a'
        if (musicUrl.includes('.wav')) return '.wav'
        return '.mp3'
    }
    ...
    }): Promise<boolean>
    {
        ...
        try {
            ...
            // 创建信息子目录
            const infoDir = path.join(savePath, 'music_infos')
            ...
            // 创建歌曲信息目录
            const songInfoDir = path.join(infoDir, fileNameBase)
            if (!fs.existsSync(songInfoDir)) {
                fs.mkdirSync(songInfoDir, { recursive: true })
            }
            ...
            if (this.checkMusicExists(musicInfo, savePath)) {
                ...
                const possibleExtensions = ['.mp3', '.flac', '.m4a', '.wav']
                for (const ext of possibleExtensions) {
                    const musicFilePath = path.join(savePath, fileNameBase + ext)
            ...
            if (musicInfo.cover) {
                ...
                if (coverFileName) {
                    // 只保存相对于音乐存储目录的路径
                    const relativePath = path.relative(savePath, path.join(songInfoDir, coverFileName))
    ...
    // 扫描已下载的歌曲
    static scanDownloadedMusic(savePath: string): DownloadedMusicItem[] {
        const fs = require('fs')
        const path = require('path')

        const downloadedSongs: DownloadedMusicItem[] = []
        ...
        try {
            ...
            for (const audioFile of audioFiles) {
                ...
                if (metadata) {
                    ...
                    // 优先使用本地封面
                    let localCoverValue: string | null = null
    ...
    // 删除下载的歌曲
    static deleteDownloadedMusic(savePath: string, songId: string): boolean {
        const fs = require('fs')
        const path = require('path')
